...
> **This repository is the Odin port of Mica.** It is a work in progress: the
> language, runtime, persistence, and HTTP/SSE host all run here. See the
> [Implementation Notes](mdbook/src/runtime/implementation-notes.md) for the
...
The wrapper builds the Odin host, starts it with the MUD fileins, and prints the
local `/mud` URL to open.
...
Keep a world in a store directory and evaluate against it:

```sh
odin run tools/filein -- --store world-db --unit equipment \
  --checkpoint apps/examples/equipment-service.mica
odin run tools/filein -- --store world-db --eval 'return ReadyForUse(#sensor_17)'
```

Start a REPL against a store or a fresh in-memory world:
...
authority checks, and a browser host surface.

This repository is the Odin port. It includes:

- the value model, relation kernel, compiler, register VM, scheduler, and runtime;
...
[Implementation Notes](mdbook/src/runtime/implementation-notes.md) for the remaining differences
...
./filein --store world-db --unit equipment apps/examples/equipment-service.mica
```

The store persists durable relations, rules, methods, and identity names, and writes a checkpoint
on clean shutdown. Later commands boot the same world and can evaluate source against it:

```sh
./filein --store world-db --actor alice --eval 'return ReadyForUse(#sensor_17)'
```

The [guide](mdbook/src/SUMMARY.md) walks through the examples, the language, and the runtime.

## Repository Map

- [`mica/var`](mica/var): Mica values, tuples, comparison, display, and copying.
- [`mica/kernel`](mica/kernel): relation storage, snapshots, transactions, rules, dispatch matching,
  and catalogue facts.
- [`mica/vm`](mica/vm): bytecode format and register VM execution core.
...
- [`apps/mud`](apps/mud): browser MUD example: world model, command parsing, UI composition, and
  sync-driven views.
- [`apps/agent`](apps/agent): relation-first LLM coding agent shell reusing the MUD's sync patterns.
...
Mica is still early, but it is a correctness-sensitive language and runtime project, so the bar is
intentionally high: changes should be understandable, tested, and grounded in the current
architecture. Run the package test suites and keep them green.

File issues and send pull requests through the
[GitHub repository](https://github.com/rdaum/omica).
...
> Rust or Odin development. If this project is useful or interesting for your team, feel free to reach out.
