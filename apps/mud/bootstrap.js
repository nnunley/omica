...
function installToolWindows(mount) {
    ...
    mount.addEventListener("click", (event) => {
        ...
        if (sourcePopout && mount.contains(sourcePopout)) {
            ...
            const source = sourceHost?.querySelector(
                `.mica-source-full[data-source-key="${cssEscape(sourcePopout.dataset.sourceKey)}"]`,
            );
            if (source) {
                createSourceWindow(
                    mount,
                    `source:${sourcePopout.dataset.sourceKey}`,
                    sourcePopout.dataset.sourceTitle || "Method source",
                    source.textContent || "",
                );
            }
            event.preventDefault();
...
function installCommandSuggestions(mount) {
    ...
    mount.addEventListener("keydown", (event) => {
        ...
        const input = event.target?.id === "command"
            ? event.target
