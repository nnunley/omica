...
test_baseline_and_structured_report_round_trip :: proc(t: ^testing.T) {
	...
	testing.expect_value(t, doc.provenance.machine, "test-host")
	testing.expect_value(t, doc.counter_unit, "count/harness-operation")
	testing.expect_value(t, doc.results[0].samples[2], f64(30))
	testing.expect_value(t, doc.results[0].throughput.units_per_op, f64(0.25))
	testing.expect(t, doc.results[0].memory_available)
	testing.expect_value(t, doc.config.min_samples, DEFAULT_CONFIG.min_samples)
	input := "suite/body\t10\nsuite/body\t20\ninvalid\tNaN\nnegative\t-1\n"
