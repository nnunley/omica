...
// CUDA's join equals the CPU reference exactly, one and two key columns with
// duplicates on both sides; zero probes return no pairs.
@(test)
test_cuda_join_agrees_with_cpu :: proc(t: ^testing.T) {
	sync.mutex_lock(&cuda_tests_lock)
	defer sync.mutex_unlock(&cuda_tests_lock)
	testing.expect(t, cuda_strategy().join_equality != nil)
	// Not offered by default: no win over the CPU hash join (Stage 3).
	testing.expect_value(t, cuda_strategy().join_min_probes, 0)
	if !cuda_or_skip(t) {
		return
	}
	...
	for width in 1 ..= 2 {
		...
		got_l, got_r, res := join_pairs(cuda_strategy(), left, right, rows, context.temp_allocator)
		testing.expectf(t, res == .Completed, "width %d: %v (%v)", width, res, last_decline_reason())
		testing.expectf(t, slice.equal(got_l, want_l) && slice.equal(got_r, want_r), "width %d: CUDA join differs (%d vs %d pairs)", width, len(got_l), len(want_l))
	}
	l, r, res := join_pairs(cuda_strategy(), [][]u64{{}}, [][]u64{{1, 2}}, []u32{0, 1}, context.temp_allocator)
